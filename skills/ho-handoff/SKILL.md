...
5. Tell the user the handoff is ready as `Handoff ready: ho-handoff <slug>`. Do not launch the new session yourself.
...
The user picks up a specific handoff with `ho-handoff <slug>`. Running `ho-handoff` without a slug automatically opens the only pending handoff or shows a picker when multiple handoffs exist. A handoff is removed after OpenCode exits successfully.
