...
2. Choose a concise semantic slug based on the project and task, such as `dotfiles-session-picker`. Use a slug Hiroshi provides. Slugs must contain only lowercase letters, numbers, and single hyphens between words.
...
5. Tell Hiroshi the handoff is ready as `Handoff ready: ho-handoff <slug>`. Do not launch the new session yourself.
...
Hiroshi picks up a specific handoff with `ho-handoff <slug>`. Running `ho-handoff` without a slug automatically opens the only pending handoff or shows a picker when multiple handoffs exist. A handoff is removed after OpenCode exits successfully.
